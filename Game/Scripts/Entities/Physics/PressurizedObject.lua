...
Net.Expose {
	...
	ClientMethods = {
		ClStartLeak = { RELIABLE_UNORDERED, NO_ATTACH, INT16, VEC3, VEC3 }, --nCX
		ClStopLeak = { RELIABLE_UNORDERED, NO_ATTACH, INT16 },
	},
	ServerMethods = {
	},
	ServerProperties = {
...
function PressurizedObject.Client:ClStartLeak(slot, pos, dir)
	local s = self:LoadObject(-1, "dummy"); -- dummy particle effect
	self:DrawSlot(s, 0);
...
function PressurizedObject.Client:ClStopLeak(slot)
	self:LoadObject(slot, "dummy"); -- dummy particle effect
...
function PressurizedObject:UpdateLeaks(frameTime)
	...
	for i,v in ipairs(self.leakInfo) do
		self:UpdateLeak(i, frameTime, v, vecNormalize(vecScale(self.gravity, -1)));
	end
end


----------------------------------------------------------------------------------------------------
function PressurizedObject:UpdateLeak(i, frameTime, leak, gravityDir)
	...
	if (leaking or self.pressure>0) then
		...
		if (self.volume <=0) then
			...
			self:StartLeaking(i, leak);
		end
	else
		self:StopLeaking(i, leak);
	end
end
...
function PressurizedObject:AddLeak(pos, dir)
	...
	self.slotCounter = (self.slotCounter or 0) + 1;
