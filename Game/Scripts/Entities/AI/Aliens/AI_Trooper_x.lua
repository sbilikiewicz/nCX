...
if (g_deadTrooperShells == nil) then
	...
	g_deadTrooperShells.explosionProbability = AI_Trooper_x.autoDestructionProbability / 100
	g_deadTrooperShells.maxNum = AI_Trooper_x.maxDeadShellNum
end
...
function AI_Trooper_x:CreateAttachments()
...
function AI_Trooper_x:OnResetCustom()
...
--function AI_Trooper_x.Server:OnUpdate(frameTime)
...
function AI_Trooper_x.Client:OnUpdate(frameTime)
...
function AI_Trooper_x:PlayAccelerationSound()
...
function AI_Trooper_x:Bleed(frameTime)
...
function AI_Trooper_x:Reset()
	BasicAlien.Reset(self);
	self:ResetTimers();
end


function AI_Trooper_x:Kill(ragdoll, shooterId, weaponId)
...
function AI_Trooper_x:ResetTimers()
...
function AI_Trooper_x.Client:OnHit(hit, remote)
...
------------------------------------------------------------------------------
function AI_Trooper_x:GetDamageMultiplier(hit)
...
------------------------------------------------------------------------------
--function AI_Trooper_x:GetDamageDistanceMultiplier(hit)
--
--	if(hit.weapon and hit.weapon.class and hit.weapon.class ~="") then 
...
function AI_Trooper_x:GetDamage()
	-- used for melee attack
	return random(40,60);
end


function AI_Trooper_x.OnDeathCustom(self)
--	self:RemoveActor();
end


-----------------------------------------------------------------------------------------------------
function AI_Trooper_x:MakeParalyzed( dir, strength )
...
--function AI_Trooper_x:Event_StartAmbush(sender)
--	if(sender) then 
--		if(sender.Who) then 
...
function AI_Trooper_x.WarmupAutoDestruct(entity)--,timerid)
		local params =
		{
			SpawnPeriod			= 0,
			Scale						= 1,
...
--	CopyVector(pos,entity:GetPos());
	--entity.iAutoDestructTimer = Script.SetTimer(entity.AutoDestructionTime*1000,AI_Trooper_x.AutoDestruct,entity);
	--entity.iWarningTimer = Script.SetTimer(500,AI_Trooper_x.SendWarning,entity);
	entity:SetTimer(TROOPER_AUTODESTRUCT_TIMER,entity.AutoDestructionTime*1000);
	entity:SetTimer(TROOPER_WARNING_TIMER,500);
end

function AI_Trooper_x.SendWarning(entity,timerid)
...
function AI_Trooper_x.PlayFightSound(entity,timerid)
...
function AI_Trooper_x:InitiateAutoDestruction()
	self:SetTimer(TROOPER_WARMUP_AUTODESTRUCT_TIMER,self.WarmupAutoDestructionTime*1000);
	--self.iAutoDestructTimer = Script.SetTimer(self.WarmupAutoDestructionTime*1000,AI_Trooper_x.WarmupAutoDestruct,self);
	self.bAutoDestructing = true;
end

function AI_Trooper_x.AutoDestruct(entity)--,timerid)
...
--function AI_Trooper_x:Cloak(cloak)
--	BasicAlien.Cloak(self,cloak);
--end

function AI_Trooper_x:SetGroupFireModes()
...
function AI_Trooper_x:MeleeAttack(entity)
...
function AI_Trooper_x:GetMeleeDamageImpulse(impulse,dirtype)
	if(dirtype==1) then 
...
function AI_Trooper_x:MeleeDamage(impulse,meleeType)
	...
	if (entity) then 	  
		...
		if(distance) then
			...
			--AI.LogEvent( "AI_Trooper_x:MeleeDamage()" );
...
--			end			
			...
			if (g_gameRules and g_gameRules.Client) then -- and g_gameRules.server.RequestMeleeHit) then
				...
				--AI.LogComment( "AI_Trooper_x:MeleeDamage: Hit "..self:GetName().."." );
--				if(entity.vehicle) then
...
function AI_Trooper_x.Client:OnTimer(timerId,mSec)
...
function AI_Trooper_x:SetFireMode()
...
function AI_Trooper_x:OnLoad(saved)
...
function AI_Trooper_x:OnSave(save)
...
function AI_Trooper_x:SetGrabbable(value)
	self.Properties.Damage.bNoGrab = 1 - (value or 0);
end

function AI_Trooper_x:Beam(target)
...
function AI_Trooper_x:RegisterWithExplosion(entity)
...
function AI_Trooper_x:NotifyExplosion()
...
-----------------------------------------------------------------------------------------------------
function AI_Trooper_x:GetDamageImpulseMultiplier(hit)
...
function AI_Trooper_x:Event_Jump(params,pos)
	Trooper_Jump(self,pos,true,true);
	self.actor:SetParams({useLandEvent = true});
end

function AI_Trooper_x:SetJumpSpecialAnim(animType,inputType,inputValue)
	self.actor:SetParams({specialAnimType = animType, specialAnimAGInput = AGInputType, specialAnimAGInputValue = inputValue});
end


function AI_Trooper_x:Event_EnableCloaked(params)
	self:Event_Enable(params);
	self:Event_Cloak();
end

--function AI_Trooper_x:Event_EnablePhysics(params)
--	self.actor:SetPhysicalizationProfile("ragdoll");
--end
--
--function AI_Trooper_x:Event_DisablePhysics(params)
--	self.actor:SetPhysicalizationProfile("unragdoll");
--end


function AI_Trooper_x:AnimationEvent(event,value)
	if ( event == "Jump" ) then
...
AI_Trooper_x.FlowEvents =
{
	Inputs =
	{
		EnableCloaked = { AI_Trooper_x.Event_EnableCloaked, "bool" },
		Cloak = { BasicAlien.Event_Cloak, "bool" },
		JumpAt = { AI_Trooper_x.Event_Jump, "vec3" },
--		EnablePhysics = { AI_Trooper_x.Event_EnablePhysics, "bool" },
--		DisablePhysics = { AI_Trooper_x.Event_DisablePhysics, "bool" },
	},
	Outputs =
	{
		EnableCloaked = "bool",
