...
-----------------------------------------------------------------------------------------------------
function AI_Scout_x:OnResetCustom()
...
function AI_Scout_x.Client:OnUpdate(frameTime)
...
function AI_Scout_x:UpdateArmor(frameTime)
	

end

function AI_Scout_x:OnShoot()
...
function AI_Scout_x:ResetAnimation()
	self:StopAnimation(0,3);
end

function AI_Scout_x:DoPlayerSeen()
...
function AI_Scout_x:DoShootWeapon()
	self:PushEvent("moac_attack");
	if (self.voiceTable) then
	end
end
	
function AI_Scout_x:DoShootWeaponLong()
	self:PushEvent("moac_attack_long");
	if (self.voiceTable) then
	end
end
	
function AI_Scout_x:DoInvestigate()
...
--	Script.SetTimer(3000, AI_Scout_x.OnDeath, self);				
...
function AI_Scout_x:ScriptEvent(event,value,str)		
...
function AI_Scout_x:GetDamageMultiplier(hit)
...
function AI_Scout_x.Client:OnHit(hit, remote)
...
function AI_Scout_x.Server:OnHit(hit)--(shooterId, weaponId, matName, damage, hitType, pos, normal, partId, radius)
    ...
    System.LogAlways("AI_Scout_x:OnHit xxx");
...
function AI_Scout_x:GrabEntity(entityname)
...
function AI_Scout_x:InitiateAutoDestruction()
	if ( self.iAutoDestructTimer == 0 ) then
	  self.iAutoDestructTimer = 1;
	  Script.SetTimerForFunction( self.warmupAutoDestructionTime*1000, "AI_Scout_x.WarmupAutoDestruct",self);
	end
end

function AI_Scout_x:WarmupAutoDestruct()			
	...
	if ( self.iAutoDestructTimer == 1 ) then
		...
		if ( self.waterCrash ~= nil) then
			...
			if ( self.waterCrash > 3000 ) then
				...
				Script.SetTimerForFunction(self.autoDestructionTime*1000,"AI_Scout_x.AutoDestruct",self);
			else
			  Script.SetTimerForFunction( 200, "AI_Scout_x.WarmupAutoDestruct",self);
			end
...
function AI_Scout_x:AutoDestruct()
...
function AI_Scout_x:ReApplyImpulse()
...
function AI_Scout_x:PushEntities(hit)
...
function AI_Scout_x.Client:OnCollision(hit)
...
-----------------------------------------------------------------------------------------------------
function AI_Scout_x:MakeParalyzed( dir, strength )
...
function AI_Scout_x.OnDeath( entity )
--	AI.LogEvent(" >>>> AI_Scout_x.OnDeath "..entity:GetName());
...
function AI_Scout_x:GrabObject(object, query)
...
------------------------------------------------------------------------------
function AI_Scout_x:EnableSearchBeam(bEnable)  
  self.actor:CreateCodeEvent({event="searchBeam", enable=bEnable});  
end

------------------------------------------------------------------------------
function AI_Scout_x:SetSearchBeamDir(worldDir)  
  self.actor:SetSearchBeam(worldDir);    
end


function AI_Scout_x:GrabMultiple()
	...
	--System.Log("AI_Scout_x:GrabMultiple() for "..self:GetName().."/"..numGrabbed)
...
function AI_Scout_x:DropObject(throw,throwVec,throwDelay)
...
function AI_Scout_x:DropMultiple(throw, throwVec, throwDelay)
...
function AI_Scout_x:OnSave(save)
...
function AI_Scout_x:OnLoad(save)
